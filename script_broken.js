...
function saveCertificateChanges() {
        if (!supabase) {
            ...
            // Cerrar ventana si se abrió
            if (window.downloadWindow) {
                window.downloadWindow.close();
                window.downloadWindow = null;
            }
            currentlyDownloading.delete(fileName); // Limpiar lock
            alert('Error: Conexión a la base de datos no disponible');
            return;
        }
        console.log('✅ Supabase disponible, obteniendo URL...');
        
        // Obtener URL de descarga desde Supabase con timeout
        console.log('💾 Solicitando URL firmada para:', fileName);
        
        // Mostrar notificación de carga
        const loadingNotification = document.createElement('div');
        loadingNotification.id = 'download-loading-' + Date.now();
        loadingNotification.innerHTML = `
            ...
            ">
                <div style="margin-bottom: 10px; font-weight: bold;">💾 Preparando Descarga...</div>
                <div style="margin-bottom: 10px; font-size: 12px; line-height: 1.4;">Obteniendo enlace seguro de Supabase...</div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <div style="
                        ...
                        animation: spin 1s linear infinite;
                    "></div>
                    <span style="font-size: 12px;">Esto puede tardar hasta 30 segundos...</span>
                </div>
            </div>
        `;
        
        // Agregar CSS para la animación del spinner
        ...
        console.log('🔄 Notificación de carga mostrada');
        
        let downloadData, error;
        try {
            // Crear promesa con timeout de 10 segundos
            const supabasePromise = supabase.storage
                .from('certificates')
                .createSignedUrl(fileName, 60);
            
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Timeout de Supabase')), 30000) // 30 segundos
            );
            ...
            console.log('🔍 Respuesta de Supabase:', { data: !!downloadData, error: !!error });
            
            // Remover notificación de carga
            if (loadingNotification && loadingNotification.parentElement) {
                loadingNotification.remove();
                console.log('🧡 Notificación de carga removida');
            }
        } catch (timeoutError) {
            console.error('⏰ Timeout en solicitud de Supabase:', timeoutError);
            error = timeoutError;
            
            // Remover notificación de carga
            if (loadingNotification && loadingNotification.parentElement) {
                loadingNotification.remove();
                console.log('🧡 Notificación de carga removida (timeout)');
            }
        }
        ...
        if (error) {
            ...
            // Mostrar notificación de error con opción de reintentar
            const isTimeout = error.message && error.message.includes('Timeout');
            const errorTitle = isTimeout ? '⏰ Conexión Lenta' : '❌ Error de Descarga';
            const errorMsg = isTimeout ? 
                'Supabase está tardando más de lo normal. Intenta esperar un momento o recargar la página.' : 
                `Error: ${error.message || 'Problema de conexión'}`;
            const bgColor = isTimeout ? '#f59e0b' : '#dc2626'; // Amarillo para timeout, rojo para error
            
            console.log('📟 Mostrando notificación de error:', { isTimeout, errorTitle, errorMsg });
            ...
            errorNotification.innerHTML = `
                <div style="
                    ...
                    border: 2px solid rgba(255,255,255,0.3);
                ">
                    <div style="margin-bottom: 10px; font-weight: bold;">${errorTitle}</div>
                    <div style="margin-bottom: 15px; font-size: 12px; line-height: 1.4;">${errorMsg}</div>
                    <div style="display: flex; gap: 8px; margin-bottom: 10px;">
                        <button onclick="this.parentElement.parentElement.parentElement.remove(); downloadSpecificCertificate('${fileName}')" 
                                style="
                                    background: ${isTimeout ? '#d97706' : '#ef4444'};
                                    color: white;
                                    padding: 8px 12px;
                                    border: none;
                                    border-radius: 4px;
            ...
            console.log('✅ Notificación de error agregada al DOM');
            
            setTimeout(() => {
                if (errorNotification.parentElement) {
                    errorNotification.remove();
                    console.log('🧡 Notificación de error auto-removida');
                }
            }, 20000); // 20 segundos para timeouts
            return;
        }
        
        if (!downloadData || !downloadData.signedUrl) {
            console.error('❌ No se obtuvo URL de descarga válida');
            // Cerrar ventana si se abrió
            if (window.downloadWindow) {
                window.downloadWindow.close();
                window.downloadWindow = null;
            ...
            currentlyDownloading.delete(fileName); // Limpiar lock
            alert('Error: No se pudo generar el enlace de descarga');
            return;
        }
        
        console.log('🔗 URL de descarga generada:', downloadData.signedUrl);
        
        const downloadLink = downloadData.signedUrl;
        const cleanFileName = fileName.split('/').pop() || 'certificado.pdf';
        
        // Método 1: Usar ventana abierta inmediatamente (preserva user gesture)
        if (window.downloadWindow && !window.downloadWindow.closed) {
            console.log('🎆 Redirigiendo ventana a descarga (user gesture preservado)');
            window.downloadWindow.location.href = downloadLink;
            
            // Cerrar ventana después de un momento para evitar que se quede abierta
            setTimeout(() => {
                if (window.downloadWindow && !window.downloadWindow.closed) {
                    window.downloadWindow.close();
                }
            ...
            console.log('✅ Descarga automática iniciada exitosamente');
        } else {
            console.log('⚠️ Ventana no disponible, usando método de respaldo');
            // Método de respaldo: enlace directo
            try {
                const link = document.createElement('a');
                ...
                console.log('✅ Intento de descarga de respaldo realizado');
            } catch (downloadError) {
                console.warn('⚠️ Descarga de respaldo fallida:', downloadError);
            }
        }
        
        // Método 3: Mostrar enlace de respaldo
        const backupMessage = `💾 Si la descarga no inició, haz clic aquí: `;
        ...
        backupLink.innerHTML = `
            <div style="
                ...
                font-family: Arial, sans-serif;
            ">
                <div style="margin-bottom: 10px;">${backupMessage}</div>
                <a href="${downloadData.signedUrl}" 
                   target="_blank" 
                   download="${fileName.split('/').pop() || 'certificado.pdf'}"
                   style="
                       color: #3DA7A0;
        ...
        // Auto-remover el mensaje después de 15 segundos
        setTimeout(() => {
            if (backupLink.parentElement) {
                backupLink.remove();
            }
        }, 15000);
        ...
        console.log('🔓 Lock liberado para:', fileName);
    }
}
...
async function deleteSpecificCertificate(fileName) {
    try {
        const confirmDelete = confirm('¿Estás seguro de que quieres eliminar este certificado? Esta acción no se puede deshacer.');
        ...
        console.log('✅ Certificado eliminado exitosamente:', fileName);
...
function saveCertificateChanges() {
    ...
    if (uploadedFile) {
        // Si se subió un archivo, actualizar la referencia en la base de datos
...
async function updateCertificateReference() {
    try {
        ...
        // Actualizar la fila actual con la referencia del archivo
